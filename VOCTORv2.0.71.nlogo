...
to initialHamlet
  ifelse regular = TRUE [
    ...
    let hamlet local_cities with[localtype = "hamlet"]
    layout-circle hamlet 20
...
to updateWinePrice
  if Sys_eco = 0 [ ;;Marché logistqiue
    ...
    ;;Si le nombre de patches céréal actuelle est sup  au nb de patch précedent -> diminution du prix
    ifelse oldAreaCereal < count patches with [typeCrop = 1][
...
165
276
S'il est \"on\" les hameau sont roganiser de manière régulière dans l'espace
9
0.0
1
...
810
381
coefDowbprince s'utilise avec logistique
9
0.0
1
